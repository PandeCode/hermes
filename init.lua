...
package.preload["fnl.autocmds"] = package.preload["fnl.autocmds"] or function(...)
  ...
  vim.opt.shadafile = _18_()
  local function _19_()
    return pcall(vim.treesitter.start)
  end
  vim.api.nvim_create_autocmd({"BufEnter", "BufWrite", "BufWritePost", "BufRead"}, {callback = _19_})
  ...
  vim.api.nvim_create_autocmd("InsertEnter", {pattern = "*", callback = _21_})
  local function _22_()
    vim.opt.relativenumber = true
    return nil
  end
  vim.api.nvim_create_autocmd("InsertLeave", {pattern = "*", callback = _22_})
  ...
  vim.api.nvim_create_user_command("Gitadd", _23_, {})
  local function _24_()
    local filename = vim.fn.expand("%")
    vim.cmd("!chmod +x %")
    return vim.notify(("Given execution rights to '" .. filename .. "'"))
  end
  vim.api.nvim_create_user_command("Chmodx", _24_, {})
  local function _25_()
    return vim.cmd("!rm -f %")
  end
  vim.api.nvim_create_user_command("Rmf", _25_, {})
...
package.preload["fnl.plugins"] = package.preload["fnl.plugins"] or function(...)
  ...
  vim.api.nvim_create_autocmd({"BufEnter", "BufWrite", "BufWritePost"}, {callback = _27_})
  ...
  vim.keymap.set("n", "[c", _28_, {silent = true})
  ...
  mini_s.setup({mappings = {add = "ys", delete = "ds", find = "", find_left = "", highlight = "", replace = "cs", update_n_lines = "", suffix_last = "", suffix_next = ""}, search_method = "cover_or_next", custom_surroundings = {f = {input = ts_input({outer = "@call.outer", inner = "@call.inner"})}, b = {input = ts_input({outer = "@block.out", inner = "@block.inner"})}, [")"] = {output = {left = "( ", right = " )"}}, ["*"] = {input = _31_, output = _32_}}})
  vim.keymap.del("x", "ys")
  ...
  vim.api.nvim_create_autocmd({"BufWritePre"}, {pattern = "*", callback = _33_})
  ...
  require("blink.cmp").setup(({signature = {enabled = true, window = {show_documentation = true}}, completion = {menu = {draw = {treesitter = {"lsp"}, columns = {{"kind_icon"}, {"label", "label_description", gap = 1}, {"kind"}}, components = {kind_icon = {text = _34_, highlight = _35_}, kind = {highlight = _36_}}}}, documentation = {auto_show = true}}} or {}))
  ...
  local no_problems
  local function _37_(params)
    local diagnostics = {}
    for i, line in ipairs(params.content) do
      for _, problem in ipairs(problems) do
        local _local_38_ = line:find(problem.pattern)
        local col = _local_38_[1]
        local end_col = _local_38_[2]
  ...
  no_problems = {method = null_ls.methods.DIAGNOSTICS, filetypes = {"*"}, generator = {fn = _37_}}
  ...
  vim.api.nvim_create_autocmd("BufWritePre", {pattern = "*", callback = _40_})
  local function _41_()
    return lsp_format_with_fallback()
  end
  vim.keymap.set({"n", "v"}, "<leader>cf", _41_)
  ...
  n("gr", _43_, "[G]oto [R]eferences")
  local function _44_()
    return snacks.picker.lsp_implementations()
  end
  n("gI", _44_, "[G]oto [I]mplementation")
  local function _45_()
    return snacks.picker.lsp_symbols()
  end
  n("<leader>lds", _45_, "[D]ocument [S]ymbols")
  local function _46_()
    return snacks.picker.lsp_workspace_symbols()
  end
  n("<leader>ws", _46_, "[W]orkspace [S]ymbols")
  local function _47_()
    return vim.lsp.inlay_hint.enable(not vim.lsp.inlay_hint.is_enabled())
  end
  n("<leader>ei", _47_, "Toggle Inlay")
  ...
  return vim.api.nvim_create_autocmd("FileType", {pattern = {"*"}, callback = _49_})
end
require("fnl.plugins")
...
package.preload["fnl.statusline"] = package.preload["fnl.statusline"] or function(...)
  ...
  local function lsp()
    ...
    if (count.errors ~= 0) then
      _61_ = ("%#" .. "DiagnosticSignError" .. "#" .. tostring((" " .. count.errors)) .. "%*")
    else
      _61_ = ""
    end
    local _63_
    if (count.warnings ~= 0) then
      _63_ = ("%#" .. "DiagnosticSignWarn" .. "#" .. tostring((" " .. count.warnings)) .. "%*")
    else
      _63_ = ""
    end
    local _65_
    if (count.hints ~= 0) then
      _65_ = ("%#" .. "DiagnosticSignHint" .. "#" .. tostring((" " .. count.hints)) .. "%*")
    else
      _65_ = ""
    end
    local _67_
    if (count.info ~= 0) then
      _67_ = ("%#" .. "DiagnosticSignInfo" .. "#" .. tostring((" " .. count.info)) .. "%*")
    else
      _67_ = ""
    end
    return (_61_ .. _63_ .. _65_ .. _67_ .. "%#Normal#")
  end
  local function git()
    ...
    if d then
      ...
      if (d.added and (d.added > 0)) then
        _69_ = ("%#" .. "GitSignsAdd" .. "#" .. tostring(("+ " .. d.added .. " ")) .. "%*")
      else
        _69_ = ""
      end
      local _71_
      if (d.changed and (d.changed > 0)) then
        _71_ = ("%#" .. "GitSignsChange" .. "#" .. tostring(("~ " .. d.changed .. " ")) .. "%*")
      else
        _71_ = ""
      end
      local _73_
      if (d.removed and (d.removed > 0)) then
        _73_ = ("%#" .. "GitSignsRemove" .. "#" .. tostring(("- " .. d.removed .. " ")) .. "%*")
      else
        _73_ = ""
      end
      return (" " .. d.head .. " " .. _69_ .. _71_ .. _73_)
    else
      return ""
    end
  end
  ...
  local function mode()
    ...
    local _let_78_ = (modes[m] or {"?", "base05", "base00"})
    local text = _let_78_[1]
    local fg = _let_78_[2]
    local bg = _let_78_[3]
  ...
  Statusline.active = function()
    ...
    return (mode() .. " " .. recording() .. " " .. git() .. " " .. filename() .. " " .. fun() .. " " .. lsp() .. "%=" .. lsp_progress() .. " " .. searchcount() .. "%=" .. wordcount() .. " " .. nix_shell() .. " " .. _98_ .. " " .. "%{&filetype != '' ? &filetype : 'text'} " .. get_attached_clients() .. " " .. "[%P %l:%c]")
  end
...
package.preload["fnl.tabline"] = package.preload["fnl.tabline"] or function(...)
  ...
  Tabline.render = function()
    ...
    for i, buf in ipairs(bufs) do
      ...
      result = (result .. "%#" .. hl .. "# " .. _121_ .. icon .. " " .. name .. _123_ .. "%*" .. buf_diag(buf))
    end
  ...
  for i = 1, 9 do
    ...
    vim.keymap.set("n", ("<leader>" .. i), _125_, {desc = ("Go to buffer " .. i)})
  end
  ...
  vim.api.nvim_create_autocmd({"BufAdd", "BufDelete", "BufEnter"}, {callback = _127_})
...
vim.api.nvim_create_autocmd("ModeChanged", {group = vim.api.nvim_create_augroup("diagnostic_redraw", {}), callback = _134_})
...
local function eval_fennel(code)
  local case_138_, case_139_ = pcall(require, "fennel")
  if ((case_138_ == true) and (nil ~= case_139_)) then
    local fennel = case_139_
    local case_140_, case_141_ = pcall(fennel.eval, code)
    if ((case_140_ == true) and (nil ~= case_141_)) then
      local result = case_141_
      return vim.inspect(result)
    elseif ((case_140_ == false) and (nil ~= case_141_)) then
      local err = case_141_
      return tostring(err)
    else
      return nil
    end
  elseif ((case_138_ == false) and true) then
    local _ = case_139_
...
ft_eval = {lua = {n = _146_, v = _147_}, fennel = {n = _148_, v = _149_}}
...
vim.keymap.set("n", "<leader>ee", _151_, {desc = "Eval"})
local function _152_()
  return eval_dispatch("v")
end
vim.keymap.set("v", "<leader>ee", _152_, {desc = "Eval selection"})
return nil
