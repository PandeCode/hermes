...
package.preload["fnl.utils"] = package.preload["fnl.utils"] or function(...)
  ...
  Utils.bind_tmux = function(binding, cmd)
    local function _14_()
      return vim.fn.jobstart({"sh", "-c", ("tmux split-window -l 10 '" .. cmd .. " && exit 0 || cat'; tmux last-pane")})
    end
    return vim.keymap.set("n", binding, _14_)
  end
  Utils.bind_job = function(binding, cmd)
    local function _15_()
      return vim.fn.jobstart({"sh", "-c", cmd})
    end
    return vim.keymap.set("n", binding, _15_)
  end
  Utils.bind_term = function(binding, cmd)
    return vim.keymap.set("n", binding, Utils.bind_tmp_term("<leader>mu", "make upload"))
  end
  return Utils.bind_term
end
require("fnl.utils")
package.preload["fnl.options"] = package.preload["fnl.options"] or function(...)
  ...
  vim.api.nvim_create_autocmd("Filetype", {pattern = {"wgsl", "glsl"}, callback = _16_})
...
package.preload["fnl.autocmds"] = package.preload["fnl.autocmds"] or function(...)
  ...
  vim.api.nvim_create_autocmd("BufReadPost", {callback = _25_})
  local function _27_()
    return pcall(vim.treesitter.start)
  end
  vim.api.nvim_create_autocmd({"BufEnter", "BufWrite", "BufWritePost", "BufRead"}, {callback = _27_})
  ...
  vim.api.nvim_create_autocmd({"CursorMoved", "DiagnosticChanged"}, {group = vim.api.nvim_create_augroup("diagnostic_only_virtlines", {}), callback = _34_})
  local function _40_()
    return pcall(vim.diagnostic.show)
  end
  vim.api.nvim_create_autocmd("ModeChanged", {group = vim.api.nvim_create_augroup("diagnostic_redraw", {}), callback = _40_})
  local function _41_()
  ...
  return vim.api.nvim_create_autocmd("BufWritePost", {pattern = "*", callback = _41_})
end
...
package.preload["fnl.plugins"] = package.preload["fnl.plugins"] or function(...)
  ...
  vim.api.nvim_create_autocmd({"BufEnter", "BufWrite", "BufWritePost"}, {callback = _44_})
  ...
  mini_s.setup({mappings = {add = "ys", delete = "ds", find = "", find_left = "", highlight = "", replace = "cs", update_n_lines = "", suffix_last = "", suffix_next = ""}, search_method = "cover_or_next", custom_surroundings = {f = {input = ts_input({outer = "@call.outer", inner = "@call.inner"})}, b = {input = ts_input({outer = "@block.out", inner = "@block.inner"})}, [")"] = {output = {left = "( ", right = " )"}}, ["*"] = {input = _48_, output = _49_}}})
  vim.keymap.del("x", "ys")
  ...
  vim.api.nvim_create_autocmd({"BufWritePre"}, {pattern = "*", callback = _50_})
  require("blink.indent").setup((nil or {}))
  local function _51_(ctx)
    local kind_icon, _, _0 = MiniIcons.get("lsp", ctx.kind)
    return kind_icon
  end
  local function _52_(ctx)
    local _, hl, _0 = MiniIcons.get("lsp", ctx.kind)
    return hl
  end
  local function _53_(ctx)
    local _, hl, _0 = MiniIcons.get("lsp", ctx.kind)
    return hl
  end
  require("blink.cmp").setup(({fuzzy = {implementation = "prefer_rust"}, signature = {enabled = true, window = {show_documentation = true}}, completion = {menu = {draw = {treesitter = {"lsp"}, columns = {{"kind_icon"}, {"label", "label_description", gap = 1}, {"kind"}}, components = {kind_icon = {text = _51_, highlight = _52_}, kind = {highlight = _53_}}}}, documentation = {auto_show = true}}} or {}))
  ...
  return vim.api.nvim_create_autocmd("FileType", {pattern = {"*"}, callback = _55_})
end
require("fnl.plugins")
...
package.preload["fnl.statusline"] = package.preload["fnl.statusline"] or function(...)
  ...
  local function lsp()
    ...
    if (count.errors ~= 0) then
      _67_ = ("%#" .. "\239\129\151 " .. "#" .. tostring(count.errors) .. "%*")
    else
      _67_ = ""
    end
    local _69_
    if (count.warnings ~= 0) then
      _69_ = ("%#" .. "\239\129\177 " .. "#" .. tostring(count.warnings) .. "%*")
    else
      _69_ = ""
    end
    local _71_
    if (count.hints ~= 0) then
      _71_ = ("%#" .. "\239\129\154 " .. "#" .. tostring(count.hints) .. "%*")
    else
      _71_ = ""
    end
    local _73_
    if (count.info ~= 0) then
      _73_ = ("%#" .. "\239\129\153 " .. "#" .. tostring(count.info) .. "%*")
    else
      _73_ = ""
    end
    return (_67_ .. _69_ .. _71_ .. _73_ .. "%#Normal#")
  end
  local function git()
    ...
    if d then
      ...
      if (d.added and (d.added > 0)) then
        _75_ = ("%#" .. "GitSignsAdd" .. "#" .. tostring(("+ " .. d.added .. " ")) .. "%*")
      else
        _75_ = ""
      end
      local _77_
      if (d.changed and (d.changed > 0)) then
        _77_ = ("%#" .. "GitSignsChange" .. "#" .. tostring(("~ " .. d.changed .. " ")) .. "%*")
      else
        _77_ = ""
      end
      local _79_
      if (d.removed and (d.removed > 0)) then
        _79_ = ("%#" .. "GitSignsDelete" .. "#" .. tostring(("- " .. d.removed .. " ")) .. "%*")
      else
        _79_ = ""
      end
      return (("%#" .. "GitSignsAdd" .. "#" .. tostring((" \238\156\165 " .. d.head)) .. "%*") .. " " .. _75_ .. _77_ .. _79_)
    else
      return ""
    end
  end
  ...
  local function mode()
    ...
    local _let_84_ = (modes[m] or {"?", "base05", "base00"})
    local text = _let_84_[1]
    local fg = _let_84_[2]
    local bg = _let_84_[3]
  ...
  local function nix_shell()
    ...
    if env then
      local _88_
      do
        local pv_89_, pv_90_ = MiniIcons.get("os", "nixos")
        local icon_2_auto,hl_3_auto = pv_89_, pv_90_
        local _91_
        if hl_3_auto then
          _91_ = ("%#" .. hl_3_auto .. "#")
        else
        ...
        _88_ = (_91_ .. (icon_2_auto or "") .. "%*")
      end
      return (_88_ .. ("%#" .. "Comment" .. "#" .. tostring(env) .. "%*"))
    else
      return ""
    end
  end
  local function filename()
    ...
    if vim.bo.modified then
      _98_ = ("%#" .. "WarningMsg" .. "#" .. tostring("\226\151\143") .. "%*")
    else
      _98_ = ""
    end
    local _100_
    if vim.bo.readonly then
      _100_ = ("%#" .. "DiagnosticError" .. "#" .. tostring("\240\159\148\146") .. "%*")
    else
      _100_ = ""
    end
    return ("%#Comment#" .. _94_ .. "%*" .. (_96_ .. ((icon .. " " .. name) or "") .. "%*") .. " " .. _98_ .. _100_)
  end
  Statusline.active = function()
    local _102_
    do
      local pv_103_, pv_104_ = MiniIcons.get("file", (vim.fn.expand("%") or "default"))
    ...
    return (mode() .. " " .. recording() .. " " .. git() .. " " .. filename() .. " " .. lsp() .. "%=" .. get_attached_clients() .. "%=" .. searchcount() .. " " .. fun() .. " " .. nix_shell() .. " " .. _102_ .. " " .. "%{&filetype != '' ? &filetype : 'text'} " .. " " .. wordcount() .. " " .. "[%P %l:%c]")
  end
  ...
  vim.api.nvim_create_autocmd({"WinEnter", "BufEnter"}, {group = group, callback = _107_})
  local function _108_()
    vim.opt_local.statusline = "%!v:lua.Statusline.inactive()"
    return nil
  end
  vim.api.nvim_create_autocmd({"WinLeave", "BufLeave"}, {group = group, callback = _108_})
  local function _109_()
    return vim.cmd.redrawstatus()
  end
  return vim.defer_fn(_109_, 1000)
end
...
package.preload["fnl.tabline"] = package.preload["fnl.tabline"] or function(...)
  ...
  vim.api.nvim_create_autocmd({"TermRequest", "ModeChanged"}, {desc = "Refresh tabline", callback = _110_})
  Tabline = {}
  do
    local p = MiniBase16.config.palette
  ...
  Tabline.render = function()
    ...
    for i, buf in ipairs(bufs) do
      ...
      result = (("%#" .. hl0 .. "#" .. tostring(icon) .. "%*") .. result .. "%#" .. hl0 .. "# " .. _127_ .. " " .. name .. status .. "%*" .. buf_diag(buf))
    end
  ...
  for i = 1, 9 do
    ...
    vim.keymap.set("n", ("<leader>" .. i), _129_, {desc = ("Go to buffer " .. i)})
  end
  ...
  vim.api.nvim_create_autocmd({"BufAdd", "BufDelete", "BufEnter"}, {callback = _131_})
  ...
  return vim.keymap.set("n", "<leader>tt", _132_, {desc = "Toggle tabline"})
end
require("fnl.tabline")
...
package.preload["fnl.lsp"] = package.preload["fnl.lsp"] or function(...)
  ...
  local no_problems
  local function _134_(params)
    local diagnostics = {}
    for i, line in ipairs(params.content) do
      for _, problem in ipairs(problems) do
        local _local_135_ = line:find(problem.pattern)
        local col = _local_135_[1]
        local end_col = _local_135_[2]
  ...
  no_problems = {method = null_ls.methods.DIAGNOSTICS, filetypes = {"*"}, generator = {fn = _134_}}
  ...
  vim.api.nvim_create_autocmd("BufWritePre", {pattern = "*", callback = _137_})
  local function _138_()
    return lsp_format_with_fallback()
  end
  vim.keymap.set({"n", "v"}, "<leader>cf", _138_)
