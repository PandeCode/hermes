...
package.preload["fnl.autocmds"] = package.preload["fnl.autocmds"] or function(...)
  ...
  vim.api.nvim_create_autocmd("BufReadPost", {callback = _22_})
  local function _24_()
    return pcall(vim.treesitter.start)
  end
  vim.api.nvim_create_autocmd({"BufEnter", "BufWrite", "BufWritePost", "BufRead"}, {callback = _24_})
  vim.api.nvim_create_autocmd("TextYankPost", {callback = vim.hl.on_yank})
  local function _25_()
    return vim.fn.mkdir(vim.fn.expand("<afile>:p:h"), "p")
  end
  vim.api.nvim_create_autocmd("BufWritePre", {pattern = "*", callback = _25_})
  vim.opt.number = true
  ...
  vim.api.nvim_create_autocmd("InsertEnter", {pattern = "*", callback = _26_})
  local function _27_()
    vim.opt.relativenumber = true
    return nil
  end
  vim.api.nvim_create_autocmd("InsertLeave", {pattern = "*", callback = _27_})
  ...
  vim.api.nvim_create_user_command("Gitadd", _28_, {})
  local function _29_()
    local filename = vim.fn.expand("%")
    vim.cmd("!chmod +x %")
    return vim.notify(("Given execution rights to '" .. filename .. "'"))
  end
  vim.api.nvim_create_user_command("Chmodx", _29_, {})
  local function _30_()
    return vim.cmd("!rm -f %")
  end
  vim.api.nvim_create_user_command("Rmf", _30_, {})
  ...
  vim.api.nvim_create_autocmd({"CursorMoved", "DiagnosticChanged"}, {group = vim.api.nvim_create_augroup("diagnostic_only_virtlines", {}), callback = _31_})
  local function _37_()
    return pcall(vim.diagnostic.show)
  end
  return vim.api.nvim_create_autocmd("ModeChanged", {group = vim.api.nvim_create_augroup("diagnostic_redraw", {}), callback = _37_})
end
...
package.preload["fnl.plugins"] = package.preload["fnl.plugins"] or function(...)
  ...
  vim.api.nvim_create_autocmd({"BufEnter", "BufWrite", "BufWritePost"}, {callback = _39_})
  ...
  vim.keymap.set("n", "[c", _40_, {silent = true})
  ...
  mini_s.setup({mappings = {add = "ys", delete = "ds", find = "", find_left = "", highlight = "", replace = "cs", update_n_lines = "", suffix_last = "", suffix_next = ""}, search_method = "cover_or_next", custom_surroundings = {f = {input = ts_input({outer = "@call.outer", inner = "@call.inner"})}, b = {input = ts_input({outer = "@block.out", inner = "@block.inner"})}, [")"] = {output = {left = "( ", right = " )"}}, ["*"] = {input = _43_, output = _44_}}})
  vim.keymap.del("x", "ys")
  ...
  vim.api.nvim_create_autocmd({"BufWritePre"}, {pattern = "*", callback = _45_})
  require("blink.pairs").setup((nil or {}))
  require("blink.indent").setup((nil or {}))
  local function _46_(ctx)
    local kind_icon, _, _0 = MiniIcons.get("lsp", ctx.kind)
    return kind_icon
  end
  local function _47_(ctx)
    local _, hl, _0 = MiniIcons.get("lsp", ctx.kind)
    return hl
  end
  local function _48_(ctx)
    local _, hl, _0 = MiniIcons.get("lsp", ctx.kind)
    return hl
  end
  require("blink.cmp").setup(({signature = {enabled = true, window = {show_documentation = true}}, completion = {menu = {draw = {treesitter = {"lsp"}, columns = {{"kind_icon"}, {"label", "label_description", gap = 1}, {"kind"}}, components = {kind_icon = {text = _46_, highlight = _47_}, kind = {highlight = _48_}}}}, documentation = {auto_show = true}}} or {}))
...
package.preload["fnl.statusline"] = package.preload["fnl.statusline"] or function(...)
  ...
  local function lsp()
    ...
    if (count.errors ~= 0) then
      _62_ = ("%#" .. "\239\129\151 " .. "#" .. tostring(count.errors) .. "%*")
    else
      _62_ = ""
    end
    local _64_
    if (count.warnings ~= 0) then
      _64_ = ("%#" .. "\239\129\177 " .. "#" .. tostring(count.warnings) .. "%*")
    else
      _64_ = ""
    end
    local _66_
    if (count.hints ~= 0) then
      _66_ = ("%#" .. "\239\129\154 " .. "#" .. tostring(count.hints) .. "%*")
    else
      _66_ = ""
    end
    local _68_
    if (count.info ~= 0) then
      _68_ = ("%#" .. "\239\129\153 " .. "#" .. tostring(count.info) .. "%*")
    else
      _68_ = ""
    end
  ...
  local function git()
    ...
    if d then
      ...
      if (d.removed and (d.removed > 0)) then
        _74_ = ("%#" .. "GitSignsDelete" .. "#" .. tostring(("- " .. d.removed .. " ")) .. "%*")
      else
        _74_ = ""
      end
      return (("%#" .. "GitSignsAdd" .. "#" .. tostring((" \238\156\165 " .. d.head)) .. "%*") .. " " .. _70_ .. _72_ .. _74_)
    else
      return ""
    end
  end
  ...
  local function nix_shell()
    ...
    if env then
      local _83_
      do
        local pv_84_, pv_85_ = MiniIcons.get("os", "nixos")
        local icon_2_auto,hl_3_auto = pv_84_, pv_85_
        local _86_
        if hl_3_auto then
          _86_ = ("%#" .. hl_3_auto .. "#")
        else
        ...
        _83_ = (_86_ .. (icon_2_auto or "") .. "%*")
      end
      return (_83_ .. ("%#" .. "Comment" .. "#" .. tostring(env) .. "%*"))
    else
      return ""
    end
  end
  local function filename()
    ...
    if vim.bo.modified then
      _93_ = ("%#" .. "WarningMsg" .. "#" .. tostring("\226\151\143") .. "%*")
    else
      _93_ = ""
    end
    local _95_
    if vim.bo.readonly then
      _95_ = ("%#" .. "DiagnosticError" .. "#" .. tostring("\240\159\148\146") .. "%*")
    else
      _95_ = ""
    end
    return ("%#Comment#" .. _89_ .. "%*" .. (_91_ .. ((icon .. " " .. name) or "") .. "%*") .. " " .. _93_ .. _95_)
  end
  Statusline.active = function()
    local _97_
    do
      local pv_98_, pv_99_ = MiniIcons.get("file", (vim.fn.expand("%") or "default"))
    ...
    return (mode() .. " " .. recording() .. " " .. git() .. " " .. filename() .. " " .. lsp() .. "%=" .. get_attached_clients() .. "%=" .. searchcount() .. " " .. fun() .. " " .. nix_shell() .. " " .. _97_ .. " " .. "%{&filetype != '' ? &filetype : 'text'} " .. " " .. wordcount() .. " " .. "[%P %l:%c]")
  end
  ...
  vim.api.nvim_create_autocmd({"WinEnter", "BufEnter"}, {group = group, callback = _102_})
  local function _103_()
    vim.opt_local.statusline = "%!v:lua.Statusline.inactive()"
    return nil
  end
  vim.api.nvim_create_autocmd({"WinLeave", "BufLeave"}, {group = group, callback = _103_})
  local function _104_()
    return vim.cmd.redrawstatus()
  end
  return vim.defer_fn(_104_, 1000)
end
...
package.preload["fnl.tabline"] = package.preload["fnl.tabline"] or function(...)
  ...
  vim.api.nvim_create_autocmd({"TermRequest", "ModeChanged"}, {desc = "Refresh tabline", callback = _105_})
  Tabline = {}
  do
    local p = MiniBase16.config.palette
...
package.preload["fnl.lsp"] = package.preload["fnl.lsp"] or function(...)
  ...
  local no_problems
  local function _129_(params)
    local diagnostics = {}
    for i, line in ipairs(params.content) do
      for _, problem in ipairs(problems) do
        local _local_130_ = line:find(problem.pattern)
        local col = _local_130_[1]
        local end_col = _local_130_[2]
  ...
  no_problems = {method = null_ls.methods.DIAGNOSTICS, filetypes = {"*"}, generator = {fn = _129_}}
  ...
  vim.api.nvim_create_autocmd("BufWritePre", {pattern = "*", callback = _132_})
  local function _133_()
    return lsp_format_with_fallback()
  end
  vim.keymap.set({"n", "v"}, "<leader>cf", _133_)
